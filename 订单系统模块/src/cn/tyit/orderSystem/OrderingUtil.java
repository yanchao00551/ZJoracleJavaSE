...
public class OrderingUtil {
	...
	public void initial(){
		...
		oSet.times[1] = 18;
		oSet.addresses[1] = "天府路";
		oSet.sumPrice[1] = 20.0;
		oSet.states[1] = 0;
	}
    ...
    void delete() {
		...
		for(int i=0; i<oSet.names.length; i++){
			...
			}else if(oSet.names[i] != null && oSet.states[i] == 0 && delId == i + 1){    //状态值为已预订， 序号值为用户输入的序号：不可删除
    ...
    void sign() {
		...
		for(int i=0; i<oSet.names.length;i++){
			...
			if(oSet.names[i]!=null && oSet.states[i] == 0 && signOrderId == i+1){
				oSet.states[i] = 1;  //将状态值设置为1
				System.out.println("订单签收成功！");
				isSignFind = true;
				
			}else if(oSet.names[i] != null && oSet.states[i] == 1 && signOrderId == i + 1) { //如果状态为已完成，序号为用户输入的订单序号：不可签收
	...
	boolean add() {
		...
		for(int j=0;j<oSet.names.length;j++){
			if(oSet.names[j] == null){    //找到第一个空位置，可以在这个位置添加订单信息
				isAdd = true;  //可以订餐 并且标志
				...
				while(time < 10 || time > 20){
					System.out.print("您 的输入有误，请输入10`20间的整数！");
					time = input.nextInt();
				}
				...
				//无需添加订单状态  默认为0   即已预订状态
				System.out.print("订餐成功！");
				System.out.print("您订的是： "+dishMeg);
				System.out.print("送餐时间： " + time + "时");
				System.out.print("餐费： "+sumPrice+"元，送餐费" + deliCharge +"元，总计："+(sumPrice + deliCharge) + "元");
				...
				oSet.sumPrice[j] = sumPrice + deliCharge;
				break;  //添加完数据后  直接终止循环
			}
		}
		
		if(!isAdd){
			System.out.print("对不起，您的餐袋已满！");
			return false;
		}else{
			return true;
		}
