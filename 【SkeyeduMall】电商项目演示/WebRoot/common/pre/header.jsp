  ...
  <div class="sou">
    <!--Begin 所在收货地区 Begin-->
        ...
        <span class="fr">
          ...
          <!-- 1) 如果session会话中loginUser对象的值为空，用户是未登录状态，然后加入Servlet路径和执行的action方法 -->
          ...
          <!-- 2) 如果session会话中loginUser对象的值不为空，用户是以登录状态，如果不是系统管理员，则显示用户姓名，然后链接到用户管理中心  -->
          <c:if test="${sessionScope.loginUser!=null}">
          ...
          <!--3) 如果session会话中对象的值不为空（已登录），且 是系统管理员 ，则进入系统总后台 -->
          ...
          <!-- 4)如果loginUser对象不是null ，已登录显示注销链接 -->
           <c:if test="${sessionScope.loginUser!=null}">
